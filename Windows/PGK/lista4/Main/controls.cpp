...
void Controls::computeMatricesFromInputs(){
	...
	ProjectionMatrix = glm::perspective(glm::radians(FoV), (float)ysize/ysize, 0.1f, 100.0f);
	// Camera matrix
...
void Controls::computeMatricesFromInputs(glm::vec3& objectPos){
	...
	// Projection matrix : 45� Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	ProjectionMatrix = glm::perspective(glm::radians(FoV), (float)xsize/ysize, 0.1f, 100.0f);//xsize/2
	// Camera matrix
	//std::cout << position[0] << "\t" << position[1] << "\t" << position[2] << "\n";
	ViewMatrix       = glm::lookAt(
								position,           // Camera is here
