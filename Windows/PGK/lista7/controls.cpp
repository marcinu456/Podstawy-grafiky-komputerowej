...
void Controls::computeMatricesFromInputs(glm::vec3& objectPos) {
	...
	if (active) {
		...
		// GlutMouseWheelFunc can't  be found
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS) {
			float zoom = mouseSpeed * float(ysize / 2 - ypos);
			if (zoom > 0) {
				this->radius = ((this->radius - 1.0) * (1 + abs(zoom))) + 1.0;
			}
			else if (zoom < 0) {
				this->radius = ((this->radius - 1.0) / (1 + abs(zoom))) + 1.0;
			}
	...
	// Projection matrix : 45� Field of View, 4:3 ratio, display range : 0.1 unit <-> 100 units
	ProjectionMatrix = glm::perspective(glm::radians(FoV), (float)xsize / ysize, 0.001f, 100.0f);
	// Camera matrix
	//std::cout << position[0] << "\t" << position[1] << "\t" << position[2] << "\n";
	ViewMatrix = glm::lookAt(
		position,           // Camera is here
