...
Sphere::Sphere(int N, bool planet = true):
...
void Sphere::draw(const glm::mat4& MVP, const glm::vec3& cam) {
    bindProgram();
    bindBuffers();
    glUniform4f(0, pos[0], pos[1], pos[2], 0);
    glUniform4f(1, scale[0], scale[1], scale[2], 1);
    glUniform3f(2, cam.x, cam.y, cam.z);
    if (!planet) glUniform3f(3, 1, 1, 1);
    else glUniform3f(3, 0, 0, 1);
    glUniformMatrix4fv(4, 1, GL_FALSE, &MVP[0][0]);
    if (!planet) glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glDrawArrays(GL_TRIANGLES, 0, g_vertex_buffer_data.size() / 3); // 12*3 indices starting at 0 -> 12 triangles
    if (!planet) glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
}
