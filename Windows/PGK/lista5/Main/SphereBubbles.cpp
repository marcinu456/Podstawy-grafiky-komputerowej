#include "SphereBubbles.h"


double generator(double n, double N)
{
    static std::random_device rd;
    static std::mt19937 generator(rd());
    static std::uniform_real_distribution<double> dis(0.0, 1.0);

    return dis(generator) * (N - n) + n;

}


SphereBubbles::SphereBubbles(const size_t& N) :
    g_vertex_buffer_data(generateMesh(50)),
    ...
    N(N)
{
    ...
    for (int i = 0; i < sizeInstances; i++) {
        float sc = generator(1 / (double)N, 0.5 / (double)N) / 4;
        this->scale[i][0] = sc;
        this->scale[i][1] = sc;
        this->scale[i][2] = sc;
        this->pos[i][0] = generator(-1 + this->scale[i][0], 1 - this->scale[i][0]);
        this->pos[i][1] = generator(-1 + this->scale[i][1], 1 - this->scale[i][1]);
        this->pos[i][2] = generator(-1 + this->scale[i][2], 1 - this->scale[i][2]);
...
void SphereBubbles::move(float dt) {
    for (int i = 0; i < sizeInstances; i++) {
...
std::vector<GLfloat> SphereBubbles::generateMesh(int N) {
...
std::vector<GLfloat> SphereBubbles::generateNormals(int N, std::vector<GLfloat> points) {
...
SphereBubbles::~SphereBubbles() {
    delete[] pos;
    delete[] scale;
}

void SphereBubbles::setShaders() {
    compileShaders(R"END(
        ...
        void main(){
            ...
            if( acos(dot(normal, L2) / length(normal) / length(L2)) > 3.141593/2.0){
                vcol = vec3(1,1.0,1.0) * (length(dot(normal,L2)));
                vcol *= ((vpos[1]+1) * 0.9);
            }
            else{
            ...
            if( acos(dot(normal, L) / length(normal) / length(L)) < 3.141593/2.0){
                ...
                vcol += vec3(0,0.5,0) * (length(dot(normal,L)))*(1-R);
            }
...
void SphereBubbles::draw(glm::mat4& MVP, const glm::vec3& light) {
    bindProgram();
    bindBuffers();
