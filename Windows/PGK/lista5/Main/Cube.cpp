...
Cube::Cube() :
    scale(1, 1, 1)
{
    GLfloat cross_color[3] = { 1, 0.0, 0.0 };
    setShaders();
    setBuffers();
}
...
void Cube::setShaders() {
    compileShaders(R"END(
        ...
        layout(location = 2)uniform float vertexColor;
        ...
        void main(){
            vec3 ocol = vec3(fragmentColor,vpos[1],1);
            color = vec4(ocol,1);
...
void Cube::draw(glm::mat4& MVP,float color) {
    bindProgram();
    bindBuffers();
