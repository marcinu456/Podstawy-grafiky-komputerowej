...
std::vector<GLfloat> Sphere::generateMesh(int N){
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        for (int j = 0; j < N * 2; ++j)
        {
            ...
            std::vector<GLfloat> tVer;

            if (i != 0) {
                tVer.push_back(radius * cosf(stackAngle) * cosf(sectorAngle));
                tVer.push_back(radius * sinf(stackAngle));
                tVer.push_back(radius * cosf(stackAngle) * sinf(sectorAngle));

                tVer.push_back(radius * cosf(stackAngle + stackStep) * cosf(sectorAngle));
                tVer.push_back(radius * sinf(stackAngle + stackStep));
                tVer.push_back(radius * cosf(stackAngle + stackStep) * sinf(sectorAngle));

                tVer.push_back(radius * cosf(stackAngle) * cosf(sectorAngle + sectorStep));
                tVer.push_back(radius * sinf(stackAngle));
                tVer.push_back(radius * cosf(stackAngle) * sinf(sectorAngle + sectorStep));
            }
            if (i != (N - 1)) {
                tVer.push_back(radius * cosf(stackAngle) * cosf(sectorAngle + sectorStep));
                tVer.push_back(radius * sinf(stackAngle));
                ...
                tVer.push_back(radius * cosf(stackAngle + stackStep) * cosf(sectorAngle + sectorStep));
                tVer.push_back(radius * sinf(stackAngle + stackStep));
                tVer.push_back(radius * cosf(stackAngle + stackStep) * sinf(sectorAngle + sectorStep));
            }

            vertices.insert(vertices.end(), tVer.begin(), tVer.end());
        }
    }
    ...
    compileShaders(R"END(
        ...
        void main(){
            ...
            if( acos(dot(normal, L) / length(normal) / length(L)) > 3.141593/2.0){
                ...
                vcol = vec3(0,0.5,0) * 0.7 * (length(dot(normal,L)));
                vcol *= ((vpos[1]+1) * 0.9);
            }
        }
    )END", R"END(
